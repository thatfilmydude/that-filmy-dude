...
async function getReview(slug) {
  try {
    const res = await fetch(
      process.env.STRAPI_URL + "/api/reviews?filters[slug][$eq]=" + slug + "&populate=*",
      {
        headers: {
...
function renderBlocks(blocks) {
  ...
  return blocks.map(function (block, i) {
    if (block.type === "paragraph") {
      ...
      return (
        <p key={i} className="mb-6 text-[17px]">
          {text}
        </p>
      );
    }
    ...
    if (block.type === "heading") {
      const text = (block.children || [])
        ...
        .join("");
      return (
        <h2 key={i} className="font-display text-3xl text-cream mt-10 mb-4 tracking-wide">
          {text}
        </h2>
      );
    }
    ...
    if (block.type === "quote") {
      ...
      return (
        <blockquote
          key={i}
          className="border-l-2 border-gold pl-5 py-1 italic text-gold text-xl my-8"
        >
          {text}
        </blockquote>
      );
    ...
    if (block.type === "image") {
      return (
        <img
          ...
          className="rounded-lg my-8 w-full border border-cream/10"
        />
      );
    }
...
function CrewRow({ label, value }) {
  ...
  return (
    <div className="flex flex-col">
      <span className="font-mono text-[10px] tracking-widest uppercase text-muted mb-1">
        {label}
      </span>
    ...
    <main className="min-h-screen bg-backdrop">
      <div className="max-w-3xl mx-auto px-8 md:px-0">
        <div className="relative pt-10 pb-12">
          <Link
            href="/#reviews"
            className="inline-block font-mono text-[11px] tracking-widest uppercase text-gold/80 hover:text-gold transition mb-8"
          >
          ...
          <div className="flex flex-col sm:flex-row gap-8 items-start">
            {posterUrl ? (
              <img
                src={posterUrl}
                alt={review.title}
                className="w-40 sm:w-48 rounded-lg border border-cream/10 shadow-2xl flex-shrink-0"
              />
            ) : (
            ...
            <div className="pt-2 flex-1">
              <p className="font-mono text-xs tracking-[0.2em] uppercase text-gold mb-3">
                {review.genre} - {review.duration}
                {releaseDate ? " - " + releaseDate : ""}
              </p>
              <h1 className="font-display text-3xl sm:text-4xl text-cream leading-[0.98] mb-6">
                {review.title}
              </h1>
        ...
        <div className="pb-16">
          <Link
            href="/#reviews"
            className="inline-block font-mono text-xs tracking-widest uppercase border border-cream/20 text-cream px-6 py-3 rounded-sm hover:border-gold hover:text-gold transition"
          >
